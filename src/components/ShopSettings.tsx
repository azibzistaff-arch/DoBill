...
export default function ShopSettings() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      {/* Tab Selectors */}
      <div className="flex overflow-x-auto custom-scrollbar bg-white p-1.5 rounded-2xl border border-slate-100 shadow-sm gap-1 w-full shrink-0">
        {ability?.can("read", "ShopDetails") && (
          <button
            onClick={() => setActiveTab("shop")}
            className={`flex-1 shrink-0 sm:shrink px-3.5 py-2.5 sm:py-3 text-xs sm:text-sm font-bold uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-1.5 whitespace-nowrap ${
              activeTab === "shop"
          ...
          <Card className="border-none shadow-xl shadow-emerald-900/10 rounded-[2rem] overflow-hidden bg-gradient-to-br from-white via-emerald-50/20 to-teal-50/30 border-2 border-emerald-500/20">
            ...
            <CardContent className="p-6 sm:p-8 space-y-6">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm space-y-5">
                ...
                <div className="pt-2 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-semibold text-slate-700">
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      checked={autoReport.sendEvenIfNoSales}
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      checked={autoReport.attachPdf}
                      onChange={(e) => setAutoReport({ ...autoReport, attachPdf: e.target.checked })}
                      className="h-4 w-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500"
                    />
                    <span>Attach Printable HTML/PDF Report</span>
                  </label>

                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      checked={autoReport.attachExcel}
                      onChange={(e) => setAutoReport({ ...autoReport, attachExcel: e.target.checked })}
                      className="h-4 w-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500"
                    />
                    <span>Attach Excel/CSV Spreadsheet</span>
                  </label>
                ...
                {/* Save Settings & Test Email Buttons */}
                <div className="pt-3 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-slate-100">
                  <Button
                    type="button"
                    onClick={() => handleTriggerAutoReportNow('daily')}
                    disabled={!!isAutoReportTriggering}
                    variant="outline"
                  ...
                  <Button
                    ...
                    className="w-full sm:w-auto h-11 px-8 font-extrabold bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl shadow-md cursor-pointer gap-2"
                  >
