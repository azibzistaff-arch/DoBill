        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          {downloadOptions.map((opt) => {
            ...
            return (
              <div
                key={opt.id}
                className={`relative bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-8 border transition-all duration-300 flex flex-col justify-between group hover:shadow-2xl hover:-translate-y-1 ${
                  isDetected 
              ...
              >
                <div>
                  {/* Top Row: FileType & Badge */}
                  <div className="flex items-center justify-between gap-2 mb-3">
                    <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-400 block truncate">
                      {opt.fileType}
                    </span>
                    {opt.badge && (
                      <span className="shrink-0 px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-wider bg-indigo-100 text-indigo-800 border border-indigo-200">
                        {opt.badge}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-3 sm:gap-4 mb-3">
                    <div className="h-11 w-11 sm:h-14 sm:w-14 rounded-xl sm:rounded-2xl bg-indigo-600 text-white shadow-lg shadow-indigo-600/30 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                      <Icon className="h-5 w-5 sm:h-7 sm:w-7" />
                    </div>
                    <div className="min-w-0">
                      <h4 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight leading-tight group-hover:text-indigo-600 transition-colors">
                        {opt.title}
                      </h4>
                    </div>
                  </div>

                  <p className="text-xs text-slate-500 font-medium leading-relaxed mb-4 sm:mb-6">
                    {opt.description}
                  </p>
                </div>
                ...
                <div className="pt-3 sm:pt-4 border-t border-slate-100 flex flex-col gap-2">
                  <a
                    ...
                    className="w-full h-11 sm:h-13 rounded-xl sm:rounded-2xl font-black text-xs sm:text-sm tracking-wide flex items-center justify-center gap-2 sm:gap-2.5 bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-600/30 active:scale-[0.98] transition-all cursor-pointer no-underline"
                  >
                    <Download className="h-4 w-4 sm:h-5 sm:w-5 shrink-0" />
                    <span className="truncate">{opt.title} ({opt.extension})</span>
                  </a>
                </div>
              </div>
            );
