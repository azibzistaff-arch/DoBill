...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-white p-4 sm:p-6 rounded-2xl sm:rounded-[2rem] shadow-sm border border-slate-100 gap-4 min-w-0">
        <div className="flex items-center gap-3 sm:gap-4 min-w-0 w-full sm:w-auto">
          <div className="h-12 w-12 sm:h-16 sm:w-16 rounded-full border border-slate-200 ring-4 ring-primary/5 overflow-hidden bg-slate-100 flex items-center justify-center shrink-0 shadow-sm">
            {userProfile.avatar ? (
              ...
              <span className="text-base sm:text-lg font-black text-slate-500">
                {userProfile.name ? userProfile.name.split(' ').map(n=>n[0]).join('').substring(0,2).toUpperCase() : 'OS'}
              </span>
            )}
          </div>
          <div className="space-y-0.5 min-w-0 flex-1">
            <h2 className="text-base sm:text-2xl font-black text-slate-800 tracking-tight truncate">Welcome Back, {userProfile.name}!</h2>
            <p className="text-[10px] sm:text-xs text-slate-500 font-bold uppercase tracking-wider flex items-center gap-1.5 min-w-0">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse shrink-0"></span>
              <span className="truncate">Operator: {userProfile.email}</span>
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 bg-emerald-50 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl sm:rounded-2xl border border-emerald-100/65 text-emerald-700 w-full sm:w-auto justify-center shrink-0">
          <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse shrink-0" />
          <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest leading-none">POS Active & Connected</span>
        </div>
      </div>
