          ...
          groupedSales.map(([date, daySales]) => (
            <div key={date} className="space-y-4">
              ...
              <Card className="border-none shadow-sm shadow-slate-200/50 overflow-hidden bg-white/70 backdrop-blur-md relative">
                {/* Desktop view table */}
                <div className="hidden lg:block overflow-x-auto custom-scrollbar w-full block scroll-smooth">
                  <Table className="min-w-[800px] w-full">
                    ...
                    <TableBody>
                      {daySales.map((sale) => (
                        ...
                        >
                          <TableCell className="pl-6 font-bold text-primary py-4 max-w-[280px]">
                            <div className="flex items-center gap-3 min-w-0">
                              <div className={`h-8 w-8 rounded-lg flex items-center justify-center text-white shadow-sm shrink-0 ${sale.paymentMode === 'cash' ? 'bg-orange-500 shadow-orange-100' : 'bg-blue-500 shadow-blue-100'}`}>
                              ...
                              <div className="flex flex-col min-w-0 flex-1">
                                ...
                                <span 
                                  className="text-xs text-slate-600 font-extrabold mt-1 leading-normal block cursor-help" 
                                  title={(sale.items || []).map(item => item.name).join(', ')}
                                >
                                  {truncate20((sale.items || []).map(item => item.name).join(', '), 20)}
                                </span>
                              </div>
                            </div>
                          </TableCell>
                    ...
                    >
                      <div className="flex items-start gap-3 min-w-0">
                        ...
                        <div className="flex flex-col min-w-0">
                          ...
                          <span className="text-[11px] font-semibold text-slate-500 mt-0.5 cursor-help" title={(sale.items || []).map(item => item.name).join(', ')}>
                            {truncate20((sale.items || []).map(item => item.name).join(', '), 20)}
                          </span>
