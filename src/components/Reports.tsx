    ...
    <div id="printable-report-area" className="space-y-6 pb-20">
      ...
      <div className="flex overflow-x-auto gap-1 border-b border-slate-200 pb-0.5 no-print">
        ...
        <button
          ...
          className={`px-5 py-3 text-xs font-black uppercase tracking-wider border-b-2 whitespace-nowrap transition-all rounded-t-xl flex items-center gap-2 ${
            activeTab === 'low-stock' 
              ? 'border-indigo-600 text-indigo-700 bg-indigo-50/40 font-black' 
              : 'border-transparent text-slate-400 hover:text-slate-700'
          }`}
        >
          ...
          {lowStockProducts.length > 0 && (
            <span className="px-2 py-0.5 text-[10px] font-extrabold bg-rose-600 text-white rounded-full inline-flex items-center justify-center min-w-[20px] h-5 shadow-xs">
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
                      ...
                      {paymentStats.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center text-xs border-b border-slate-50 pb-1.5">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-extrabold text-slate-600 cursor-help" title={item.name}>{truncate20(item.name, 20)}</span>
                          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      {categoryDistribution.map((entry, idx) => (
                        <div key={idx} className="flex items-center justify-between p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[11px] font-black uppercase text-slate-700 cursor-help" title={entry.name}>{truncate20(entry.name, 20)}</span>
                          </div>
            ...
            <Card className="border border-slate-200/80 shadow-sm bg-white text-slate-800 p-6 rounded-3xl flex flex-col justify-between">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="text-[11px] font-black tracking-widest text-emerald-700 bg-emerald-50 px-3 py-1 rounded-md border border-emerald-200/80 font-mono uppercase">
                ...
                <div className="space-y-3 pt-2">
                  <div className="flex justify-between items-center text-xs text-slate-500 font-semibold">
                    ...
                    <span className="font-mono text-slate-900 font-extrabold text-sm">₹{stats.totalRevenue.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs text-slate-500 font-semibold">
                    <span>Weaver Wholesales (Cost):</span>
                    <span className="font-mono text-slate-700 font-bold text-sm">₹{(stats.totalRevenue - stats.totalProfit).toLocaleString()}</span>
                  </div>
                  <div className="border-t border-slate-100 pt-3 mt-1 flex justify-between items-center bg-emerald-50/60 -mx-6 px-6 py-3 border-b border-emerald-100">
                    <span className="text-xs font-black text-emerald-800 tracking-wide uppercase">EST GROSS PROFIT:</span>
                    <span className="text-2xl font-black font-mono text-emerald-600">₹{stats.totalProfit.toLocaleString()}</span>
                  </div>
                </div>
              </div>
              <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl text-[11px] font-medium text-slate-600 leading-relaxed mt-4">
      ...
      {activeTab === 'product' && (
        <Card className="border-none shadow-sm shadow-slate-100">
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto custom-scrollbar w-full block scroll-smooth">
                  ...
                  {productWiseSales.map((p, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/40">
                      <td className="px-6 py-4 max-w-[260px]">
                        <div className="text-[11px] font-black text-slate-700 leading-snug cursor-help" title={p.name}>{truncate20(p.name, 20)}</div>
                        <span className="text-[9px] text-indigo-500 font-black uppercase mt-1 block truncate" title={p.category}>{truncate20(p.category, 20)}</span>
                      </td>
      ...
      {activeTab === 'low-stock' && (
        <Card className="border-none shadow-sm shadow-slate-100">
          <CardHeader className="border-b border-slate-50 flex flex-row justify-between items-center pb-4">
            ...
            {lowStockProducts.length > 0 ? (
              <span className="px-3 py-1 text-xs font-black bg-rose-600 text-white rounded-full shadow-xs inline-flex items-center gap-1">
                {lowStockProducts.length} Alert items
              </span>
            ) : (
              <span className="px-3 py-1 text-xs font-black bg-emerald-50 text-emerald-700 border border-emerald-200/80 rounded-full">
                Stock optimal
              </span>
            )}
          </CardHeader>
                ...
                <tbody className="divide-y divide-slate-50">
                  {lowStockProducts.map((p, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/40 bg-red-50/15">
                      <td className="px-6 py-4 max-w-[260px]">
                        <div className="text-[11px] font-black text-rose-700 leading-snug cursor-help" title={p.name}>{truncate20(p.name, 20)}</div>
                        <span className="text-[9px] text-slate-400 mt-1 block truncate" title={p.brand || 'Local weaver'}>Brand/Weaver: {truncate20(p.brand || 'Local weaver', 20)}</span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <code className="text-[10px] font-mono font-bold bg-slate-100 px-2 py-0.5 rounded text-slate-600">{p.barcode}</code>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className="font-mono font-black text-xs px-2.5 py-1 bg-rose-600 text-white rounded-full inline-flex items-center justify-center shadow-xs">
