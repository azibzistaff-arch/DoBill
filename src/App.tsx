...
const AppContent = () => {
      ...
      if (inviteEmail) {
        ...
        if (hasAccess) {
          ...
          try {
            ...
            console.warn('[App] history.replaceState bypassed:', histErr);
          }
        } else {
  ...
  const [savedAccounts, setSavedAccounts] = useState<SavedAccount[]>(() => {
    try {
      const raw = localStorage.getItem('retailpro_saved_accounts');
      if (raw) {
        const parsed: SavedAccount[] = JSON.parse(raw);
        const filtered = parsed.filter(a => {
          const u = (a.username || a.email || '').toLowerCase().trim();
          return u !== 'casher' && u !== 'cashier' && u !== 'default';
        });
        return filtered;
      }
    } catch (e) {}
  ...
  const handleDirectLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Login via database credentials
  ...
  const renderDownloadModal = () => {
    ...
    return (
      <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[999] flex items-center justify-center p-2 sm:p-6 animate-fade-in font-sans overflow-y-auto">
        <div className="bg-white w-full max-w-6xl rounded-2xl sm:rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden transform transition-all animate-scale-up my-auto max-h-[96vh] sm:max-h-[92vh] flex flex-col">
          {/* Header */}
          <div className="relative bg-gradient-to-br from-slate-900 via-indigo-950 to-indigo-900 p-4 sm:p-8 text-white shrink-0">
            <button 
              type="button"
              onClick={() => setShowDownloadModal(false)}
              className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-white/15 hover:bg-white/25 text-white p-2 rounded-full transition-all cursor-pointer border-none"
              style={{ border: 'none' }}
            >
              <X className="h-5 w-5" />
            </button>
            <div className="inline-flex items-center gap-1.5 text-indigo-300 bg-indigo-500/15 border border-indigo-400/20 px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-widest mb-2 sm:mb-3">
              <Sparkles className="h-3.5 w-3.5 text-indigo-400 animate-pulse" />
              DO BILL App Center (ऐप सेंटर)
            </div>
            <h3 className="text-xl sm:text-3xl font-black tracking-tight text-white pr-8">Download DO BILL App</h3>
            <p className="text-slate-300 text-xs sm:text-sm mt-1 max-w-xl">
              अपने मोबाइल या कंप्यूटर पर DO BILL को ऐप की तरह चलाएं। Smart architecture auto-detection enabled.
            </p>
          </div>

          {/* Modal Scrollable Body */}
          <div className="p-3 sm:p-8 overflow-y-auto space-y-6 custom-scrollbar">
            {/* DOWNLOAD HUB WITH AUTO DETECT & MANUAL DOWNLOAD OPTIONS */}
            <DownloadHub onClose={() => setShowDownloadModal(false)} />
          </div>
        </div>
      </div>
    );
...
export default function App() {
  // Use HashRouter for native webview / local file loading protocols / Electron to prevent blank screens
